 ...
 * naming the same ending across seeds, which is what turns it from noise into
 * a finding.
 *
 * COST, measured in the shape where it is actually paid. CI runs with
 * `COMPLETION_PLAYERS=1`, which removes the (otherwise dominant)
 * `completionProbe` entirely and leaves these ones as essentially the whole
 * job — see `.github/workflows/ci.yml`'s own note on that shape.
 */
...
function main(): void {
   ...
   * CLAUDE.md failure mode 5, a cohort-level effect read off a thin slice of a
   * population.
   *
   * `lichProbe` has existed for exactly this since issue #24 and its own doc
   * comment says so — its dedicated cohort "raises the expected count" well
   * above the population slice's — but the check was never pointed at it, so
   * the probe was computed, folded into `byEndingAnywhere` for the
   * reachability sweep, and otherwise unread (failure mode 2). The
   * rarity-ordering check below already reads the probe, which meant one
   * report printed two different lich-seeker rates from two different samples
   * and called them the same thing.
   */
