...
import type { Artifact, ArtifactPower, Condition, Effect, OfferOption, Rarity } from '../src/types';
 ...
 * still bounded because the whole ten-rung lair ladder is `DEF_LAIR * 9 = 72`,
 * and a single relic worth more than a sizeable fraction of that is a mistake
 * whatever the intent.
 */
 ...
 *
 * Failure mode 2 in its exact shape: a union member wired through `defenseOf`,
 * `decayFor`, `applyEffects` and the card renderer, that no relic in the
 * catalog actually carries, is a feature that cannot be encountered. The
