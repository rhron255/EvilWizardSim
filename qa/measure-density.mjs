 ...
 * scroll around": at 393x852 the header pushed the first choice card below
 * the fold on every era. Eyeballing a screenshot cannot tell you by how much,
 * and CLAUDE.md #7 says screenshots lie — so this measures.
 *
 ...
 *
 * Issue #36 removed the run screen's tab split (and the ledger it carried),
 * so there is no longer a second view to switch into and measure separately
 * — everything below is what a player sees on the one screen that exists now.
 *
...
// --long picks the 20-era career, the deepest history this screen can carry.
...
const run = await page.evaluate(() => {
  ...
  return {
    ...
    // the faction standings and the decision content's own status lines,
    // which is the part this pass actually controls. `firstOptionTop`
...
console.log('run (above the fold):', JSON.stringify(run, null, 2));
