...
describe('notoriety decay', () => {
  it('is zero during the ascent', () => {
    expect(decayFor({ phase: 'ascent', erasSinceProphecy: 0, isLich: false, heldArtifactIds: [] }, fixtureContent)).toBe(0);
    expect(decayFor({ phase: 'ascent', erasSinceProphecy: 5, isLich: false, heldArtifactIds: [] }, fixtureContent)).toBe(0);
  });

  it('compounds with eras since the prophecy', () => {
    const at = (n: number) => decayFor({ phase: 'decline', erasSinceProphecy: n, isLich: false, heldArtifactIds: [] }, fixtureContent);
  ...
  it('is frozen for a lich — the branch that cheats the decline', () => {
    expect(decayFor({ phase: 'decline', erasSinceProphecy: 6, isLich: true, heldArtifactIds: [] }, fixtureContent)).toBe(0);
  });
});
